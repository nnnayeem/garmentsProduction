                ...
                <div class="box box-warning">
                    ...
                    <div class="box-body">
                        <table id="example2" class="table table-bordered table-hover">
                            <thead>
                            <tr>
                                <th>#</th><th> style="align:center" Switch</th><th style="align:center">Machine Token</th><th style="align:center">Actions</th>
                            </tr>
                            </thead>
                            <tbody align="center" valign = "middle">
                            @for($i=1;$i<=$floor->rows*$floor->machinePerRow;$i++)
                            <tr>
                                <td></td>
                                <td>{{$i}}</td>
                                <td>{!! !is_null($data = $floor->machines->where('floor_id',$floor->id)->where('switch',$i)->first())? $data->machine_token:'<span class="label label-danger">Machine Not Added</span>' !!}</td>
                                <td><a href="/admin/machines/setSwitch/{{$floor->id}}/{{$i}}" class="btn btn-flat bg-purple">Set Machine</a></td>
                            </tr>
                            @endfor
                            </tbody>
                        </table>
